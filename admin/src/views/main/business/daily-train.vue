  ...
  <a-modal v-model:visible="visible" title="每日车次" @ok="handleOk"
    ...
    <a-form :model="dailyTrain" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="车次编号">
          <train-select-view v-model:model-value="dailyTrain.code" @change="onChangeCode"></train-select-view>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onChangeCode = (train) =>{
        if(JSON.stringify(train) === '{}'){
            dailyTrain.value = train
            return
        }
        let t= Tool.copy(train);
        delete t.id;
        // 用assign 可以合并 , 以t作为数据源
